 ...
 *
 */
public class Thesaurus extends NOR {
	...
	protected void loadContent() {
		...
		if (connector instanceof DBConnector) {
			...
			if (dataModel instanceof RecordBasedDataModel) {
				...
				//content = ((AdjacencyListDataModel)dataModel).loadContentFromDB(connector,sEntity,content); 
			}
		...
		if (connector instanceof XMLConnector) {
			...
			if (dataModel instanceof RecordBasedDataModel) {
				...
				for (SchemaEntity sEntity : getSchema().getSchemaEntities()) {
					...
					if (sEntity.isList())
						entities = ((RecordBasedDataModel)dataModel).loadContentFromXMLSEntityList(connector,sEntity);
					else
